...
theme.Slideshow = (function() {
  ...
  function slideshow(el) {
    ...
    this.settings = {
      ...
      dots: false,
      fade: true,
      prevArrow: this.$slideshow.data('arrowl'),
      nextArrow: this.$slideshow.data('arrowr'),
      draggable: true,
      touchThreshold: 20,
      infinite: true,
      slidesToShow: 1,
    ...
    this.$slideshow.on('beforeChange', function(event, slick, currentSlide, nextSlide){
      if (this.classList[0] == 'split-slider'){
...
$(".SHOP").click(function(){
  ...
  if($(".SHOP-item").hasClass('dropdown-hidden')){
    $(".SHOP-item").slideDown("slow").removeClass('dropdown-hidden');
    $(".SHOP-icon").css("transform","rotate(180deg)");
  }
  else{
    $(".SHOP-item").slideUp("slow").addClass("dropdown-hidden");
    $(".SHOP-icon").css("transform","");
  }
});

$(".JOURNAL").click(function(){
  if($(".JOURNAL-item").hasClass('dropdown-hidden')){
  	$(".JOURNAL-item").slideDown("slow").removeClass('dropdown-hidden');
    $(".JOURNAL-icon").css("transform","rotate(180deg)");
  }
  else{
  	$(".JOURNAL-item").slideUp("slow").addClass("dropdown-hidden");
    $(".JOURNAL-icon").css("transform","");
  }
});
...
//       function (){
//         $(this).next().addClass('border__black');
//       },
//       function (){
//         $(this).next().removeClass('border__black');
//     });

//     $(window).scroll(function() {
//       var height = $(window).scrollTop();
//       if(height  > ($("#hero").height() ) ) {
//         $(".site-header").removeClass("transparent").css("position", "fixed");
//         $(".site-nav").css("margin-left", "0");
//         $('.mobile-logo').addClass('small--hide');
//         $('.logo-align--left').removeClass('small--hide');
//         $(".site-header__logo").show();
//         $(".site-header__account").show();
//         $(".site-header__cart").show();
//       }
//       else{
//         $(".site-header").addClass("transparent").css("position", "absolute");
//         $('.mobile-logo').removeClass('small--hide');
//         $('.logo-align--left').addClass('small--hide');
...
//     $('.mobile-logo').addClass('small--hide');
  ...
  /* HC - Always black links and logo on blog page - 02 Feb '18
    $(".site-header__logo").show();
    $(".site-header__account").show();
    $(".site-header__cart").show();
    $(".site-header").removeClass("transparent").css('background-color', 'transparent');
    $('.site-header__cart--link').css('color', '#f5f5f5');
    $('.site-nav__link--main, .account__link, .site-header__cart').css('color', '#f5f5f5').addClass('transparent__hover');
    $('.site-header__logo-image img').attr('src', 'https://cdn.shopify.com/s/files/1/2245/2689/files/EUGENIA_KIM_Logo_White.png?3083436459274822176');
    $('.mobile-logo').attr('src', 'https://cdn.shopify.com/s/files/1/2245/2689/files/EUGENIA_KIM_Logo_White.png?3083436459274822176');
    $('.icon-hamburger').css('fill', '#fff');
    $('.dropdown__hover').hover( 
      function (){
        $(this).next().addClass('border__white');
        $(this).next().removeClass('border__black');
      },
      function (){
        $(this).next().removeClass('border__white');
    });
    ...
    $(window).scroll(function() {
      ...
      if(height  > 0 ) {
        $(".site-header").css('background-color', '#f5f5f5');
        $('.site-header__cart--link').css('color', '#000');
        $('.site-nav__link--main, .account__link, .site-header__cart').css('color', '#000').removeClass('transparent__hover');
        $('.site-header__logo-image img').attr('src', 'https://cdn.shopify.com/s/files/1/2245/2689/files/EUGENIA-KIM_Logo.png?9702053988255462843');
        $('.mobile-logo').attr('src', 'https://cdn.shopify.com/s/files/1/2245/2689/files/EUGENIA-KIM_Logo.png?9702053988255462843');
        $('.icon-hamburger').css('fill', '#000');
        $('.dropdown__hover').hover( 
          function (){
            if($(window).scrollTop() > 0){
...
//collection page mobile subnav dropdown
$('.collection-header__submenu').click( function(){
  $('.collection-header__dropdown').slideToggle().css('display', 'block');
  $(this).find('i').toggleClass('spin-once');
});
...
//Accordians
$('.storage-care__link').click(function(){
  $('.storage-care__dropdown').slideToggle();
});

$('.size-guide__link').click(function(){
  $('.size-guide__dropdown').slideToggle();
});
...
$(document).ready(function() {
  $('ul.tabs').each(function(){
    ...
    active = links.first().addClass('tab__active');
    content = $(active.attr('href'));
    content.addClass('story__content--active');
    links.not(':first').each(function () {
    });

    $(this).find('a').click(function(e){
      active.removeClass('tab__active');
      content.removeClass('story__content--active');
      active = $(this);
      content = $($(this).attr('href'));
      active.addClass('tab__active');
      content.addClass('story__content--active');
      return false;
    });
  });
...
//filter articles based on query string
window.onload = function() {
  //trigger click on correct filter
  var selectedFilter = '#' + window.location.href.split('?')[1];
  $(selectedFilter).trigger("click");
  //second time to close dropdown
  $(selectedFilter).trigger("click");
};

//filter articles
function filterArticles(device) {
  var selectedFilter = 'all';
  var selectedSort = 'forward';

  if(device == 'desktop'){
    //get values from custom input field
    selectedFilter = $('input[name="article-filter"]').val().toLowerCase();
    selectedSort = $('input[name="article-sort"]').val().toLowerCase();
  }
  else {
    //get values from select boxes
    if($('select[name="blog-filter"]').val()){
    selectedFilter = $('select[name="blog-filter"]').val().toLowerCase();
    }
    if ($('select[name="blog-sort"]').val()){
      selectedSort = $('select[name="blog-sort"]').val().toLowerCase();
    } 
  }
  ...
  $('.blog__square').each(function(){
    if($(this).hasClass(selectedSort)){
      if($(this).data(selectedFilter) === true){
        $(this).fadeIn('slow');  
      }  
    }
  });
...
$('.drop-menu__sort .dropeddown li').click(function () {
  var input = $(this).parents('.drop-menu').find('input').val();

  if(input.toLowerCase() == 'forward'){
    input = 'most recent';
  }
  ...
  $('.msg__sort').html(input + '</span>');
  filterArticles('desktop');

}); 


$('.drop-menu__filter .dropeddown li').click(function () {
  var input = $(this).parents('.drop-menu__filter').find('input').val();

  if(input.toLowerCase() == 'all'){
    input = 'all posts';
  }

  $('.msg__filter').html(input + '</span>');
  filterArticles('desktop');

}); 

/***********************************************/
// NAV BAR DROPDOWN EVENT LISTENERS
/***********************************************/

function resetNav(){
  $('.site-nav--active-dropdown').removeClass('site-nav--active');
  $('.site-nav--active-dropdown').removeClass('site-nav--active-dropdown');
//   $('.dropdown__hover--triangle').removeClass('dropdown__hover--triangle');
  $('.account__dropdown').hide();
...
//     $(this).children('span').addClass('dropdown__hover--triangle');
...
//show cart dropdown
$('.site-header__cart').mouseenter(
  function(){
    resetNav();
    //for mobile
    $('.site-header__menu').hide();
    $('.dropdown__closer').show();
  }
);

//show account dropdown
$('.account__link').mouseenter(
  function(){
    resetNav();
...
//hide dropdowns
$('header, .account__dropdown, .cart__dropdown').hover(
  function (){},
  function(){
    resetNav();
  }
...
//hide dropdowns when other nav links are hovered
$('.site-nav__link--main').mouseenter( 
  function(){
    resetNav();
  }
);
...
//hide cart and account dropdowns mobile
$('.dropdown__closer').click(function(){
  $('.account__dropdown').hide();
  $(this).hide();
  $('.site-header__menu').show();
});

//close cart and account dropdowns
...
$(document).ready(function(){
  ...
  // Remove "face-icon__active" class from tab links
  tablinks = document.getElementsByClassName("face-icon");
  for (i = 1; i < tablinks.length; i++) {
      tablinks[i].className = tablinks[i].className.replace(" face-icon__active", "");
  }
});


//Face Shape Tabs Controller
function openShape(evt, shapeType) {
  ...
  // Show the current tab, and add an "face-icon__active" class to the button that opened the tab
  document.getElementById(shapeType).style.display = "flex";
  evt.currentTarget.className += " face-icon__active";
} 

//custom select boxes using jQuery Selectric
$(function() {
  $('#SortBy').selectric({
    arrowButtonMarkup: '<i class="fa fa-angle-down unrotated" aria-hidden="true"></i>',
    onOpen: function() {
       $('.selectric i').toggleClass('rotated');
       $('.selectric i').toggleClass('unrotated');
    },
    onClose: function() {
       $('.selectric i').toggleClass('unrotated');
       $('.selectric i').toggleClass('rotated');
